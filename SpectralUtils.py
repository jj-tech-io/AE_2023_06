...
def compensateForCameraResponse(hsIm, cameraResponse):
    """
    Compensate for camera response on a hyperspectral image.
    :param hsIm: Hyperspectral image (H x W x C)
    :param cameraResponse: RGB camera response function (C x 3), channel order should be red, green, blue.
    :return: corrected_hsIm: Resulting hyperspectral image with compensated camera response
    """
    # Calculate normalization factors for each channel based on the camera response
    norm_factors = np.sum(cameraResponse, axis=1)
    
    # Normalize each hyperspectral channel by its respective factor
    corrected_hsIm = hsIm / norm_factors[:, None, None]
    
    return corrected_hsIm
